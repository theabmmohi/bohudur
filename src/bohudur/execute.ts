import type { BohudurOptions, ExecuteRequest, ExecuteResponse } from "@type/index"
import { Request } from "@bohudur/http"
import { toDate } from "@bohudur/helpers"

const path: string = "/execute/v2/"
...
export default async function execute(key: string, options: BohudurOptions, request: ExecuteRequest): Promise<ExecuteResponse> {
  const data = await Request<ExecuteResponseWire>(key, options, path, {
